...
<script>
    ...
    export default {
        ...
        methods:{
            goToDetail(id){
                console.log(id);
                this.$router.push({name:'contract_detail_hr',query:{id:id}});
            },
            get(){
                getContract({loginId:CommonUtils.getStore("user").userId,authorization:CommonUtils.getStore("token")},1)//1为招聘者
                    .then(res=>{
